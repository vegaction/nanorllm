...
def transform_step_samples(episode_rollout: EpisodeRollout) -> list[TrainSample]:
    train_samples = []
    for step in episode_rollout.step_views:
        input_ids = torch.concat([step.prompt_ids, step.response_ids], dim=0)
        loss_mask = torch.concat([torch.zeros_like(step.prompt_ids), torch.ones_like(step.response_ids)], dim=0)
        old_logprobs = torch.concat([torch.zeros_like(step.prompt_ids), step.response_logprobs], dim=0)
        train_samples.append(TrainSample(input_ids=input_ids,
                                         loss_mask=loss_mask,
...
def transform_episode_samples(episode_rollout: EpisodeRollout, tokenize_messages) -> TrainSample:
    prompt_ids = tokenize_messages(
        episode_rollout.trajectory.steps[-1].prompt_messages,
    ...
    for trajectory_step, step_view in zip(
        episode_rollout.trajectory.steps,
        episode_rollout.step_views,
        strict=True,
    ):
        step_prompt_ids = tokenize_messages(
            trajectory_step.prompt_messages,
            add_generation_prompt=True,
        ).view(-1).detach().cpu()
        ...
        resonse_len = step_view.response_ids.shape[-1]
        if current_len > prompt_ids.shape[-1] or not torch.equal(prompt_ids[:current_len], step_prompt_ids):
            raise ValueError(
                "episode-as-sequence currently requires each step prompt_ids to be a prefix of the final prompt_ids"
            )
        loss_mask[current_len : current_len+resonse_len] = 1.0
        old_logprobs[current_len : current_len+resonse_len] = step_view.response_logprobs
    return TrainSample(input_ids=input_ids,
                                         loss_mask=loss_mask,
                                         old_logprobs=old_logprobs,
                                         advantage=episode_rollout.advantage
                                         )
